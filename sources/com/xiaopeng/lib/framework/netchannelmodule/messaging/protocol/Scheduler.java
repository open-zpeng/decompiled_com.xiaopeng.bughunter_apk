...
public class Scheduler {
    ...
    public synchronized void doCheckFailedCount() {
        tf.a(TAG, "Failed:" + this.mFailedCount);
        ...
        if (this.mFailedCount % 20 == 0) {
            if (this.mState != STATE.CONNECTING && this.mState != STATE.INITIALIZING) {
                tf.a(TAG, "Try to disconnect the connection.");
                this.mClient.disconnect();
            }
            tf.a(TAG, "Try to reopen the connection.");
            this.mClient.scheduleReopen();
        }
    }
    ...
    public synchronized void scheduleAction(STATE state, STATE state2) {
        ...
        if (i == 1) {
            ...
            if (state == state3 && getNetworkType() == this.mConnectedNetworkType) {
                this.mState = state3;
                tf.a(TAG, "Skip the network change event cause of same network type!");
            } else {
                if (state == state3) {
                    this.mClient.scheduleReopen();
