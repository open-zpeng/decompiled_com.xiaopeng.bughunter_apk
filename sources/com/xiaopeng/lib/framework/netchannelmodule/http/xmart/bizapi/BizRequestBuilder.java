...
public class BizRequestBuilder {
    ...
    private String generateSignature() {
        ...
        try {
            String a = TextUtils.isEmpty(this.mBody) ? "" : uf.a(this.mBody);
            Mac mac = Mac.getInstance("HmacSHA256");
            byte[] bytes = this.mSecretKey.getBytes(XmartV1Constants.UTF8_ENCODING);
            mac.init(new SecretKeySpec(bytes, 0, bytes.length, "HmacSHA256"));
            tf.a(TAG, "generateSignature origin:" + (value + '&' + signParameters + '&' + a));
            byte[] doFinal = mac.doFinal((value + '&' + signParameters + '&' + a).getBytes(XmartV1Constants.UTF8_ENCODING));
    ...
    private String generateXpClient() {
        String a = hg.a();
        String replaceAll = TextUtils.isEmpty(a) ? "" : a.toUpperCase().replaceAll(" ", "");
        StringBuilder sb = new StringBuilder();
    ...
    private String getAuthorization() {
        if (this.mNeedAuthorizationInfo) {
            ...
            try {
                ...
                tf.f(TAG, "getAuthorization fail : " + e);
                return null;
            }
        }
        return null;
    ...
    private String getNetworkType() {
        int b = vf.b(GlobalConfig.getApplicationContext());
        return b != 1 ? b != 2 ? b != 3 ? b != 10 ? NETWORK_TYPE_UNKNOWN : NETWORK_TYPE_WIFI : NETWORK_TYPE_4G : NETWORK_TYPE_3G : NETWORK_TYPE_2G;
    }

    private xc getRequestHeaders() {
    ...
    private static String getSignParameters(xc xcVar, String str) {
        String queryParameter;
        ArrayList arrayList = new ArrayList(xcVar.e.keySet());
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            if (queryParameterNames.contains(str2) || str2.startsWith(BizConstants.HEADER_PREFIX)) {
                if (xcVar.e.containsKey(str2)) {
                    if (gg.g() && !TextUtils.isEmpty(parse.getQueryParameter(str2))) {
                        throw new RuntimeException("Duplicate keys in header and parameters!");
                    }
                    queryParameter = xcVar.e.get(str2);
