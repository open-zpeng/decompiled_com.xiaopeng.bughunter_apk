...
public abstract class BaseHttpCounter implements Handler.Callback {
    ...
    private synchronized void debugInfo() {
        if (DEBUG) {
            tf.r(TAG, "[Http (" + this.mStatEventName + ")]: Request:" + this.mRequestCount + " Total:" + this.mTotalData + " Details:" + this.mJsonConverter.toJson(this.mTrafficDetails) + " Package:" + GlobalConfig.getApplicationSimpleName() + " Net:" + GlobalConfig.getNetworkType());
        }
    }
    ...
    public synchronized void init() {
        ...
        if (z) {
            ...
            try {
                ...
                tf.f(TAG, "Failure to load from preference due to " + e);
            }
            debugInfo();
        }
    }
