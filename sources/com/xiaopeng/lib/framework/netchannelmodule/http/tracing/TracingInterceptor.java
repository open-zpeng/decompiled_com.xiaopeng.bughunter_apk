...
public class TracingInterceptor implements Interceptor {
    ...
    private void decorateAndFinishSpan(Span span, String str, String str2, HttpUrl httpUrl, int i, Exception exc) {
        span.name(str).kind(Span.Kind.CLIENT).tag(TAG_KEY_HOST, httpUrl.host()).tag(TAG_KEY_METHOD, str2).tag(TAG_KEY_PATH, httpUrl.encodedPath()).tag(TAG_KEY_URL, httpUrl.toString()).tag(TAG_KEY_STATUS, String.valueOf(i)).tag(TAG_KEY_VID, String.valueOf(yf.h())).tag(TAG_KEY_VIN, yf.g()).tag(TAG_KEY_CDUID, yf.c()).tag(TAG_KEY_SID, yf.f()).tag(TAG_KEY_NET, String.valueOf(vf.b(GlobalConfig.getApplicationContext()))).error(exc).finish();
    }
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (!isXiaopengHost(url.host())) {
            tf.a(TAG, "isXiaopengHost(url.host()) == false, host:" + url.host());
        ...
        tf.a(TAG, "Tracing intercept operationName:" + substring + " traceId:" + hexString + " spanId:" + hexString3 + " sampled:" + sampled);
        try {
            ...
            tf.a(TAG, "Tracing intercept finish, code:" + code);
            return proceed;
        } catch (Exception e) {
            ...
            tf.a(TAG, "Tracing intercept finish, code:-1");
            throw e;
        }
    }
}
