...
public final class StorageCounter implements Handler.Callback {
    ...
    private void debugInfo() {
        if (DEBUG) {
            tf.r(TAG, "[RemoteStorage] Request:" + this.mRequestCount.get() + " Failed:" + this.mFailedCount.get() + " Succeed:" + this.mSucceedCount.get() + " Size(KB):" + this.mTrafficSize.get() + " ErrorCodes:" + getErrorCodesInString() + " Package:" + GlobalConfig.getApplicationSimpleName() + " Net:" + GlobalConfig.getNetworkType());
        }
    }
    ...
    */
    ...
    public static boolean isInternationVersion() {
        boolean c = hg.c();
        Log.i("netChannel", "isInternationVersion :\t" + c);
        return c;
    }
    ...
    private StorageCounter() {
        this.mExceedTrafficQuota = false;
        this.mHandler = new Handler(ag.g(0), this);
    }
}
