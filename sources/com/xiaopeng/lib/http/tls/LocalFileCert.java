...
public class LocalFileCert {
            ...
            if (keyManagerArr == null) {
                try {
                    if (SecurityCommon.checkSystemUid(context)) {
                        tf.l(TAG, "get cert content from file");
                        certificate = FileUtils.readTextFile(new File(CERT_PATH), 0, null);
                    } else {
                        tf.l(TAG, "get cert content from SystemDelegate");
                        certificate = ((ISystemDelegate) Module.get(ig.class).get(ISystemDelegate.class)).getCertificate();
                    }
                    ...
                    tf.t(TAG, "getKeyManagers error!", e2);
                    return null;
                }
            }
        }
    ...
    public static SSLSocketFactory getTLS2SocketFactory(Context context) {
        try {
            ...
            ff.b().a(6006, e.getLocalizedMessage());
            e.printStackTrace();
            return null;
        }
    }
                ...
                try {
                    ...
                    try {
                        try {
                            ...
                            ff.b().a(6005, e.getLocalizedMessage());
                            e.printStackTrace();
                        }
                        try {
                            open.close();
                    ...
                    ff.b().a(6005, e2.getLocalizedMessage());
                    e2.printStackTrace();
                    return null;
                }
            }
