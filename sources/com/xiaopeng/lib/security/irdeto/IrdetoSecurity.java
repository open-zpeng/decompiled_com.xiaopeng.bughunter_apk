...
public class IrdetoSecurity implements ISecurityModule {
    ...
    private void checkSaveIndividualFlag() {
        ...
        tf.f(TAG, "irdeto delete indiv files");
        clearFlag();
    }
    ...
    private void initInternal(Context context, boolean z) {
        ...
        try {
            ...
            sIsInitSuccess = true;
            tf.a(TAG, "irdeto sdk init success");
        } catch (ISFException e) {
            tf.i(TAG, e);
            int errorCode = e.getErrorCode();
            if (errorCode != 10) {
                if (errorCode == 14) {
                    tf.f(TAG, "irdeto sdk init failed : secure store error!!!!");
                    clearFlag();
                }
            } else if (gg.g()) {
                sIsInitSuccess = true;
                tf.a(TAG, "irdeto sdk init success for eng root");
            }
    ...
    private void printErrorReason(String str) {
        tf.a(TAG, str + this.sInitErrorMsg);
    }

    private static void registerISFSecureStoreReloadReceiver() {
        tf.a(TAG, "registerISFSecureStoreReloadReceiver");
        try {
            ...
            sContext.registerReceiver(new BroadcastReceiver() { // from class: com.xiaopeng.lib.security.irdeto.IrdetoSecurity.4
                ...
                public void onReceive(Context context, Intent intent) {
                    if (IrdetoSecurity.ACTION_BROADCAST_ISF_SECURE_STORE_RELOAD.equals(intent.getAction())) {
                        try {
                            tf.a(IrdetoSecurity.TAG, "registerISFSecureStoreReloadReceiver ACTION_BROADCAST_ISF_SECURE_STORE_RELOAD");
    ...
    public void asyncGetMCUSecurityKey(final ICallback<byte[], String> iCallback) {
        tf.r(TAG, "asyncGetMCUSecurityKey");
        new Thread(new Runnable() { // from class: com.xiaopeng.lib.security.irdeto.IrdetoSecurity.1
            ...
            public void run() {
                try {
                    ...
                    e.printStackTrace();
                    tf.r(IrdetoSecurity.TAG, "null mcu security key");
                    iCallback.onError("null mcu security key");
                }
            }
        }).start();
    ...
    public synchronized void asyncSaveToken(final String[] strArr, final String[] strArr2, final Runnable runnable) {
        ag.i(new Runnable() { // from class: com.xiaopeng.lib.security.irdeto.IrdetoSecurity.2
            @Override // java.lang.Runnable
            public void run() {
                try {
    ...
    public synchronized void asyncSaveTokenWithListener(final String[] strArr, final String[] strArr2, final ISecurityModule.ResultListener resultListener) {
        ag.i(new Runnable() { // from class: com.xiaopeng.lib.security.irdeto.IrdetoSecurity.3
            @Override // java.lang.Runnable
            public void run() {
                String str;
    ...
    public String decode(String str) {
        synchronized (IrdetoSecurity.class) {
            ...
            try {
                ...
                tf.a(TAG, "irdeto decode result " + isfDecrypt);
                return isfDecrypt;
            } catch (Exception e) {
                tf.i(TAG, e);
                return "";
    ...
    public String encode(String str) {
        synchronized (IrdetoSecurity.class) {
            ...
            try {
                ...
                tf.a(TAG, "irdeto encode result " + isfEncrypt);
                return isfEncrypt;
            } catch (Exception e) {
                tf.i(TAG, e);
                return "";
    ...
    public boolean isInitAndIndivSuccess() {
        synchronized (IrdetoSecurity.class) {
            if (!sIsInitSuccess) {
                tf.a(TAG, "Irdeto sdk not init " + this.sInitErrorMsg);
                return false;
            }
            try {
